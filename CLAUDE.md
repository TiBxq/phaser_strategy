...
This is a browser-based **isometric economic strategy game** built with Phaser **4.0.0** and Vite 5.2.0. (Phaser 4 API: `setTintFill()` is removed — use `setTint(color).setTintMode(Phaser.TintModes.FILL)` and restore with `setTintMode(Phaser.TintModes.MULTIPLY)`.) Canvas: 960×640px, `pixelArt: true`, FIT scale mode. All game assets are generated programmatically in `Preloader.js` via Phaser's Graphics API — no external image files are needed.
...
- **`QuestConfig.js`** — `QUESTS` array defining all quests in order. Each quest has `id`, `label`, and `tasks[]`. Each task has `id`, `label`, and `type` (`buildingPlaced` | `buildingConnected` | `workerAssigned` | `resourceProduced` | `warriorsHired` | `campCleared` | `populationReached` | `goldCollected`). `buildingPlaced` carries `configId`; `buildingConnected`/`workerAssigned` carry an optional `configId` restricting the match; `resourceProduced` carries `resource` + `amount`; `warriorsHired`/`populationReached` carry `count`; `goldCollected` carries `amount`. The final quest (`ENJOY`) has an empty `tasks[]` and acts as the terminal state.
...
- **`BuildingRenderer.js`**: Manages building sprites at depth `col + row + LAYER_BUILDING`. All buildings are **2×2** — sprites are anchored at `tileToWorld(col, row)` with origin `(0.5, 1)`. Manages farm field sprites, worker overlay sprites, ghost preview, the selection overlay, **disconnected warning icons** (`icon-no-road` sprite above each building where `isConnected === false`, rebuilt on `BUILDING_PLACED` and `BUILDING_CONNECTIVITY_CHANGED`), **starvation departure icons** (`icon-starving` orange ! sprite, `x+22, y-72` relative to building anchor, added on `VILLAGER_DEPARTED` only when `reason !== 'disconnected'`, removed when `building.residents >= building.maxResidents` on `VILLAGER_RETURNED`), and **pillage target icon** (`icon-pillage` dark-red circle with white X, `x-22, y-72`, single sprite moved between buildings). Listens to `BANDIT_PILLAGE_TARGET { buildingUid }` → `_setPillageTargetIcon(buildingUid)` positions the icon over the targeted building with a pulsing alpha tween; `null` hides it. Listens to `BANDIT_CAMP_CLEARED` → hides pillage icon. On `BUILDING_REMOVED` also destroys field sprites for every tile in the emitted `fieldTiles[]`, the no-road icon, and the starvation icon for that building.
...
- **`BuildSystem`**: `placedBuildings: Map<uid, BuildingInstance>`. All buildings use a **2×2 tile footprint**. `canPlace()` checks all 4 footprint tiles — rejects mixed heights, ramp tiles, road tiles, and a second Town Hall. `place()` marks all 4 tiles, runs `onPlace` side-effects (which populate `fieldTiles` etc.), **then** determines `building.isConnected` via `RoadSystem` so farm fields are already populated for the connectivity check. `onPlace` side-effects:
  ...
  - `demolishHard(uid, tileMap, villagerManager)` — identical to `demolish()` but skips the resource refund. Used by `BanditThreatSystem` for pillage destruction.
...
- **`QuestSystem`**: Plain class. Drives linear quest progression — one quest active at a time. Constructor takes `buildSystem`, `villagerManager`, `resourceSystem`; calls `_startQuest(0)` immediately. State: `_questIndex` (index into `QUESTS`), `_taskStates` (`Map<taskId, boolean>`, rebuilt on each quest start), and `_producedCounters` (`Map<taskId, number>` for `resourceProduced` tasks). Listens to `BUILDING_PLACED` (→ `buildingPlaced` tasks + re-check of `buildingConnected` tasks for buildings connected at placement), `BUILDING_CONNECTIVITY_CHANGED` (→ `buildingConnected` tasks, optional per-task `configId` filter), `VILLAGERS_CHANGED` (→ `workerAssigned` / `populationReached` tasks), `WARRIORS_CHANGED` (→ `warriorsHired`), `PRODUCTION_TICK` (→ `resourceProduced`), `RESOURCES_CHANGED` (→ `goldCollected`), `BANDIT_CAMP_CLEARED` (→ `campCleared`). `_completeTask(taskId)` guards against non-active and already-done tasks; when all tasks in the quest are done it emits `SHOW_NOTIFICATION`, `QUEST_COMPLETED`, then calls `_startQuest(nextIndex)`. Terminal quest (`ENJOY`, empty tasks) emits `QUEST_COMPLETED` immediately on start. Public API: `get currentQuest()`, `get activeTask` (first incomplete task, for hints), `isTaskDone(taskId)`, `isComplete()`, `getTaskProgress(taskId)`. Instantiated in `Game.js` as `this.questSystem` after `hungerSystem`, immediately followed by `this.questHintSystem = new QuestHintSystem(this.questSystem)`.
- **`BanditCampSystem`** (`src/systems/BanditCampSystem.js`): Plain class, no Phaser dependency. Tracks `campCol`, `campRow`, `isCleared`, and `claimedTiles[]` (read-only getter). `initFromMap(tileMap)` reads camp position and claimed tile list stamped during map generation. `isActive()` → true when camp placed and not yet cleared. `clear(tileMap)` sets `banditClaimed = false` and `banditCampTile = false` on all stored tiles, returns `{ clearedTiles[] }` for `MapRenderer` refresh.
- **`BanditThreatSystem`** (`src/systems/BanditThreatSystem.js`): Plain class. Activated once both trigger conditions are met: any `banditClaimed` tile is `FOG_VISIBLE` AND `villagerManager.total >= 10`. The trigger latches permanently (threat continues even if population later drops below 10). State machine: `'inactive' → 'raiding' → 'pillaging'`. Listens to `PRODUCTION_TICK`:
  - **Raiding**: steals gold each tick — 1/5/10 per cycle based on current population tiers (10+/15+/20+). Partial payment (player has less than the steal amount) still drains what's available and counts as "can't pay."
  - **Grace period**: 5 consecutive "can't pay" cycles before escalating to pillaging.
  - **Pillaging**: every 10 ticks, calls `buildSystem.demolishHard()` on a random non-Town-Hall building (no resource refund). Pre-selects the next target immediately and emits `BANDIT_PILLAGE_TARGET { buildingUid }` so the UI can show a warning icon. When the player pays the full steal amount, reverts to `'raiding'`.
  - Listens to `BUILDING_REMOVED` — if the pre-selected pillage target was manually demolished, picks a new target.
  - Listens to `BANDIT_CAMP_CLEARED` → immediately transitions to `'inactive'`, emits `BANDIT_THREAT_STATE_CHANGED { state: 'inactive' }`.
  - Emits `BANDIT_THREAT_STATE_CHANGED { state, stealAmount }` on every transition and when the steal tier changes. Emits `BANDIT_PILLAGE_TARGET { buildingUid: string|null }` when target is set or cleared.
...
- **`walkable.js`**: `isWalkable(tile)` — GRASS tile with no building. Road tiles keep `type === 'GRASS'` and no `buildingId`, so they are walkable. `isWalkableForMarch(tile)` — GRASS only, ignores `buildingId`; used by warrior march pathfinding so occupied tiles are passable at high cost. `heightMoveCost(fromTile, toTile)` — returns `FLAT_MOVE_COST` (1) for same height, `RAMP_MOVE_COST` (2) for a one-level transition via a ramp, `Infinity` for cliffs or height diff > 1. `marchMoveCost(fromTile, toTile)` — like `heightMoveCost` but adds `BUILDING_MARCH_COST` (20) when `toTile.buildingId` is set; A* naturally routes around buildings when the detour is shorter. `randomWalkableTile(tileMap, exclude?, fogSystem?)` — random walkable tile from the full grid; when `fogSystem` is provided, restricts candidates to `FOG_VISIBLE` tiles only. `randomWalkableTileNear(tileMap, centerCol, centerRow, radius)` — random walkable tile within Manhattan radius; used by warriors for bounded wander.
...
- **`WarriorEntity.js`**: Single warrior sprite. Mirrors `VillagerEntity` but uses `'sprite-warrior'` texture (red-armored 10×14 sprite). Accepts optional `fogSystem`. Stores `_homeCol/_homeRow` (Barracks anchor); picks wander targets via `randomWalkableTileNear` with `WANDER_RADIUS=4`. `MOVE_DURATION=600`, idle pause `800–2000 ms`. **March methods**: `marchTo(col, row, onArrived)` cancels the active wander timer, kills tweens, then searches for a path using `isWalkableForMarch` + `marchMoveCost` — tries the anchor, all 4 footprint tiles, and all 8 perimeter tiles of the 2×2 block in order; fires `onArrived` immediately only when no path exists at all (terrain cliffs). Each completed march step calls `fogSystem.revealAround(col, row, 1)` so warriors illuminate a narrow corridor as they advance. `marchHome()` calls `marchTo(_homeCol, _homeRow, …)` then resumes normal wandering. **Combat**: owns a `Combatant` component (`this.combat`, `WARRIOR_STATS`, red base tint); `playAttackSwing(targetCol, targetRow)` faces the target and plays a random `soldier-attack01/02` animation, returning to idle on completion; wander scheduling is suppressed while `combat.inCombat` or `combat.isDead`.
- **`WarriorRenderer.js`**: Maintains a `Map<buildingUid, WarriorEntity[]>` pool. Accepts optional `fogSystem` (passed to each `WarriorEntity`). Listens to `WARRIORS_CHANGED { buildingUid, building }` → `_syncPool()` adds/removes entities to match `building.assignedVillagers`. Spawn location: `randomWalkableTileNear` with radius 3, fallback to `randomWalkableTile`. Listens to `BUILDING_REMOVED` → destroys all entities for that uid. `WARRIOR_RECALL_REQUEST` prefers recalling a warrior that is not `combat.inCombat`. **Combat API**: `allWarriors()` (flattened across pools), `findPoolUid(entity)`, `removeEntity(entity)` (splices a dead warrior from its pool without destroying it — the death animation owns the sprite until it finishes). **`marchAllTo(...)`** / **`marchAllHome()`** as before.

### Bandits (`src/bandits/`)

- **`BanditEntity.js`**: Single wandering bandit sprite (`orc-idle`/`orc-walk` spritesheets). Mirrors `VillagerEntity` but uses `randomWalkableTileNear` with `WANDER_RADIUS=4` around the camp anchor — no fog restriction (bandits live in the dark). `refreshFogVisibility()` shows/hides the sprite based on the current tile's fog state. **Combat**: owns a `Combatant` component (`this.combat`, `BANDIT_STATS`); `holdPosition()` stops wandering when an assault begins (mid-walk steps abort at the next tile), `resumeWandering()` restores it after a failed assault; `playAttackSwing()` plays a random `orc-attack01/02` animation facing the opponent. `_engagedBy` holds the warrior locked onto this bandit (1v1 duels).
- **`BanditRenderer.js`**: Created by `Game.js` after map generation. If the camp is active: spawns one `building-bandit-camp` sprite at the camp anchor and 3 `BanditEntity` instances nearby. **Fog visibility rule**: the camp sprite is shown as soon as *any* `banditClaimed` tile enters `FOG_VISIBLE` state (not just the camp tile itself) — so the player sees the camp the moment they spot the first red territory tile. The camp sprite is directly `setInteractive()` and emits `TILE_SELECTED` with the camp tile on click, bypassing the fog tile's `disableInteractive()`. `clearCamp()` destroys all bandit entities and the camp sprite. **Combat accessors**: `bandits` (live entity list), `campSprite`, `removeBandit(entity)` (splice + destroy, called after the death animation), `destroyCampSprite()` (camp only — `CombatSystem` kills the bandits individually; plays the shared collapse dust/debris effect from `src/map/DemolitionEffect.js`, the same `spawnDemolitionEffect(scene, x, y, depth)` used by `BuildingRenderer` for building demolition).

### Combat (`src/combat/`)

- **`src/data/CombatConfig.js`**: `WARRIOR_STATS { maxHp: 40, armor: 3, damageMin: 8, damageMax: 12 }`, `BANDIT_STATS { maxHp: 40, armor: 2, damageMin: 8, damageMax: 12 }` (a 1v1 duel averages 5 warrior swings vs 4 retaliations — the warrior wins around 12/40 hp), `CAMP_STATS { maxHp: 600, armor: 0 }` (the camp building's HP pool; it never retaliates — sized so a full squad needs a sustained siege after the duels). `HIT_INTERVAL_MS` (1100, time between alternating swings), `HIT_IMPACT_MS` (350, delay from swing start to damage landing), `HEAL_INTERVAL_MS`/`HEAL_AMOUNT` (out-of-combat regen +2 hp / 3 s), `ENGAGE_RANGE` (1, Chebyshev).
- **`Combatant.js`**: Component composed into `WarriorEntity`/`BanditEntity` (host must expose `_sprite`, `_shadow`, `col`, `row`). Damage formula: `dmg = max(0, rawDamage − armor)` where the attacker's raw damage comes from `rollDamage()` — a uniform integer roll in `[damageMin, damageMax]` per swing. `takeDamage(raw)` spawns a red `-N` damage float, plays the hurt reaction, updates the HP bar, returns `true` on lethal hits and starts the death sequence; `onDeathComplete(host)` fires when it finishes. Hosts pass `idleAnim`/`hurtAnim`/`deathAnim` keys (`soldier-*` / `orc-*` spritesheets, 4 frames each for hurt/death): hurt plays the hurt animation then returns to idle; death plays the death animation then fades out. Without those keys the fallbacks are a white hit-flash (Phaser 4 FILL tint mode) and a procedural fall-over tween. `kill()` is the float-less immediate-death path (used when the camp falls). The HP bar (24×3 Graphics, green→orange→red) is shown only when injured and follows the sprite via a scene `UPDATE` listener that self-detaches when the sprite is destroyed; depth uses `LAYER_HP_BAR`. A heal timer regenerates HP when not `inCombat`. Module also exports `playSfx(scene, key, config)` (cache-guarded — silently skips missing audio) and `spawnDamageFloat(scene, x, y, amount)`.
- **`CombatSystem.js`**: Assault orchestrator, instantiated in `Game.js` as `this.combatSystem`. `startAssault()` holds all bandits in place, wires `onDeathComplete` handlers, and assigns each warrior the nearest free bandit (**strict 1v1** — `bandit._engagedBy` locks the pair; warriors track per-assault `_unreachable` sets so failed pathing tries the next bandit). Warriors without a free bandit march to the camp and swing at it. **Duels** are turn-based: attacker (always the warrior first) swings, damage lands at `HIT_IMPACT_MS`, roles swap after `HIT_INTERVAL_MS`. Duel start reveals fog radius 2 around the bandit. **Warrior death** is a permanent loss: removed from the pool, then `villagerManager.killAssigned(uid)` + `WARRIORS_CHANGED` (pool is spliced first so `_syncPool` is a no-op). **Camp attack**: each adjacent warrior swings on the camp (shake tween + damage float + persistent HP bar above the sprite; camp HP persists across failed assaults); camp-attackers re-check for free reachable bandits before every swing (bandits first). At 0 camp HP: remaining bandits are `kill()`ed, `BanditCampSystem.clear()` runs, `BANDIT_CAMP_CLEARED` fires, survivors `marchHome()`. If all warriors die → "attack failed" notification, bandits resume wandering, assault can be retried. If every living warrior stalls (no path to anything) → "path blocked" notification and clean abort.
- **Combat SFX**: `Preloader` loads optional `assets/sfx/hit.wav` and `assets/sfx/death.wav` as `sfx-hit`/`sfx-death`; playback is cache-guarded so the game works without them. Camp destruction plays the existing `sfx-destroy`.
- **Testing cheat**: `window.__combatTest(n = 5)` (defined in `Game.js`) spawns `n` warriors in a `'__CHEAT__'` pool near the camp, reveals a staging area, and calls `combatSystem.startAssault()` — skips the whole economic buildup.
...
- **`BuildingMenu`**: Bottom strip. Building buttons plus a **Road** button at the end. Buttons rendered in `MENU_ORDER` order (see BuildingConfig). Road button emits `ROAD_MODE_ENTER` / `ROAD_MODE_EXIT` instead of `BUILD_MODE_ENTER` / `BUILD_MODE_EXIT`. All buttons use the lock system (`requires` field) and show affordability colours. **Unlocks latch permanently**: `_everPlaced` set (seeded from `placedBuildings`, grown on `BUILDING_PLACED`) backs `_checkRequirements`, so demolishing/pillaging a prerequisite never re-locks dependents. Unlock chain: Town Hall → Farm/Road → Lumbermill → House/Quarry/Market → (Quarry→) Iron Mine → Smithy → Barracks; Market → Warehouse. Road button uses `ROAD_CONFIG` (imported separately from `BuildingConfig.js`). Pulses the button targeted by the active quest hint (`QUEST_HINT_CHANGED`).
...
- **`BanditAlert`**: Persistent badge positioned 42px below `HungerAlert`. Hidden when `state === 'inactive'`. Shows two lines: main label (15px bold) — orange **⚠ BANDITS RAIDING!** or red **☠ BANDITS PILLAGING!** — and a smaller subtitle (11px) **-N gold/cycle** reflecting the current steal tier. Badge background resizes to fit both lines. Both lines pulse together. Listens to `BANDIT_THREAT_STATE_CHANGED { state, stealAmount }`, `TILE_SELECTED`, `TILE_DESELECTED`.
- **`CameraHint`**: Onboarding hint shown at screen center from game start — an `icon-mouse-right` icon (18×24 mouse shape with the right button highlighted gold, generated in `Preloader._generateMouseRightIcon()`) followed by "Hold the right mouse button to move the camera", both with a pulsing alpha tween. `Game.js` emits `CAMERA_PANNED` on pointerup after a right-drag pan that moved more than 30 px (so right-click mode cancels don't count); after 3 such pans the hint fades out and destroys itself.
...
- **Warrior sprites**: `WarriorRenderer` maintains one `WarriorEntity` per `building.assignedVillagers`, synced via `WARRIORS_CHANGED`. Warriors are red-tinted soldier sprites that wander within radius 4 of the Barracks using A* + `randomWalkableTileNear`.
- **Warrior death**: a warrior killed during a bandit-camp assault is a permanent loss — `CombatSystem` removes the entity and calls `villagerManager.killAssigned()`, so the Barracks count and total population both drop.
...
- **Placement**: top-left fog corner `col/row ∈ [1..4]`, Chebyshev radius 8 from anchor `(3,3)` claims the entire `[0..11]×[0..11]` zone (144 tiles). Max reach = col/row 11 < `VIS_MIN=12` — bandit territory never touches the visible starting area. Non-bandit playable area: 576 − 144 = 432 tiles ≈ the old 20×20 total.
- **Tile fields**: `banditClaimed` (blocks road and building placement) on all 144 claimed tiles; `banditCampTile` (marks the 4-tile footprint, makes tile clickable for Attack panel) on the footprint only. Both cleared by `BanditCampSystem.clear()`.
- **Visual**: visible `banditClaimed` tiles receive a red-orange tint (`0xaa4422`) via `MapRenderer.refreshFogTile`. Tint removed on `BANDIT_CAMP_CLEARED`. Dense forest groves are pre-placed inside the bandit zone so the camp feels like a hidden forest stronghold.
- **Resource separation**: iron deposit on the **right fog edge** (`col ≥ VIS_MIN, row < VIS_MIN`); second stone outcrop on the **left fog edge** (`col < VIS_MIN, row ≥ VIS_MIN`). Neither overlaps the `[0..11]²` bandit zone — iron is always accessible before clearing the camp.
- **Attack mechanic**: clicking a `banditCampTile` (or the camp building sprite) shows `TileInfoPanel` with an **Attack Camp** button. Requires ≥ 5 total warriors across all Barracks. On click, `UI.js` calls `combatSystem.startAssault()` — warriors duel the bandits 1v1, then destroy the camp building by attacking its HP pool (see **Combat** section). Warriors use `isWalkableForMarch` + `marchMoveCost` (high-cost building passthrough) and reveal fog radius 1 around each march step (plus radius 2 around each duel). The camp clears only when its HP reaches 0; there is no fog-reveal insta-clear anymore.
- **Camp footprint walkability**: `_placeBanditCamp` explicitly sets footprint tile `type = 'GRASS'` so A* can path to the camp even if groves placed FOREST there earlier.
- **Connectivity guarantee**: map generation runs two BFS checks per attempt (up to 20 retries): a road BFS (excludes bandit tiles, enforces height continuity) for iron/stone features, and a walkable BFS (includes bandit tiles and ramps, same height rules) that verifies the camp itself is reachable for warriors.
- **Events**: `BANDIT_CAMP_ATTACK_REQUEST` (emitted by TileInfoPanel on button click), `BANDIT_CAMP_CLEARED { clearedTiles }` (emitted by CombatSystem when the camp's HP reaches 0; MapRenderer subscribes to remove tints), `BANDIT_THREAT_STATE_CHANGED { state, stealAmount }` (emitted by BanditThreatSystem on state transitions and steal-tier changes), `BANDIT_PILLAGE_TARGET { buildingUid: string|null }` (emitted when a new pillage target is selected or cleared).

### Quest System

- **Quests** are defined in `src/data/QuestConfig.js` as a frozen `QUESTS` array. Quests ship in order: *First Steps* (TH → Farm → connect → assign), *Wood Industry* (Lumbermill → connect → assign → produce 5 wood), *Establishing an Economy*, *Prepare Your Defences*, *Drive Out the Bandits*, *Prepare for the Journey*, and the terminal *Enjoy the Game!*.
- **One quest is active at a time.** `QuestSystem` tracks `_questIndex` and auto-advances when all tasks are done. `get activeTask` returns the first incomplete task of the active quest (used by `QuestHintSystem`).
- **Task types** (extensible — add a new handler in `QuestSystem` + a new `type` string in `QuestConfig`):
  ...
  | `buildingConnected` | `BUILDING_CONNECTIVITY_CHANGED` (+ re-check on `BUILDING_PLACED` for buildings placed already-connected) | a building reaches `isConnected = true`; optional `task.configId` restricts the match (omitted = any non-TH) |
  | `workerAssigned` | `VILLAGERS_CHANGED` | a building has `assignedVillagers >= 1`; optional `task.configId` restricts the match |
  | `resourceProduced` | `PRODUCTION_TICK` | cumulative `produced[task.resource]` since quest start reaches `task.amount` (counter in `_producedCounters`, surfaced via `getTaskProgress`) |
  | `warriorsHired` | `WARRIORS_CHANGED` | sum of all Barracks `assignedVillagers >= task.count` |
  | `campCleared` | `BANDIT_CAMP_CLEARED` | bandit camp destroyed |
  | `populationReached` | `VILLAGERS_CHANGED` | `villagerManager.total >= task.count` |
  | `goldCollected` | `RESOURCES_CHANGED` | money on hand `>= task.amount` |
- **Quest events** (in `EventNames.js`): `QUEST_STARTED { quest }`, `QUEST_TASK_COMPLETED { quest, task }`, `QUEST_COMPLETED { quest }`, `QUEST_HINT_CHANGED { hint }`.
...
### Quest Hint System (guided onboarding)

- **`QuestHintSystem`** (`src/systems/QuestHintSystem.js`): plain class, constructed in `Game.js` right after `QuestSystem`. Derives a single hint from `questSystem.activeTask` (first incomplete task) — purely from the task type, no extra config in QuestConfig — and emits `QUEST_HINT_CHANGED { hint }` when it changes. Exposes `get currentHint()` so UI components constructed later read the initial state. Hint payloads: `{ type: 'buildButton', configId }` (buildingPlaced), `{ type: 'roadPath', targetConfigId }` (buildingConnected), `{ type: 'assignWorker', configId }` (workerAssigned), `{ type: 'resource', resource }` (resourceProduced), or `null`.
- **Consumers**:
  - `BuildingMenu` — pulses the hinted build button (gold tint + alpha yoyo); `roadPath` pulses the Road button. Locked buttons don't pulse; `_updateLockStates` re-applies the pulse after unlock changes.
  - **`QuestHintController`** (`src/systems/QuestHintController.js`, instantiated in `Game.js` after the renderers) — world-space visuals, refreshed on hint/mode/map changes. Tracks input mode and tile selection from events itself (BUILD/ROAD_MODE_ENTER/EXIT, TILE_SELECTED/DESELECTED), so it has no dependency on Game-scene state or listener order:
    - `roadPath` + road mode: computes a suggested road path from the Town Hall to the first disconnected building of the hinted type via `aStar` (walkable = existing road or road-placeable visible grass; existing roads cost 0.1 so the suggestion rides partial roads; tries the 9 closest start/goal pairs around both buildings' owned tiles incl. farm field blocks) and shows it via `mapRenderer.showHintTiles()` (texture `tile-hint`, group alpha pulse). Already-roaded tiles are filtered out.
    - `buildButton` + build mode for the hinted config: highlights all valid placement anchors — only for placement-constrained buildings (`requiresAdjacentTo` set or `buildableOn` without GRASS, i.e. Lumbermill/Quarry/Iron Mine). `canPlace` failures with reason `'Insufficient resources.'` still count as valid spots.
    - `assignWorker`: `buildingRenderer.showQuestHintOverlay(building)` — gold pulsing copy of the building sprite at `DEPTH_SELECTION_OVERLAY`; hidden while that building is selected (blue selection overlay takes over).
  - `VillagerPanel` — pulses the `[+]` button when the shown building matches an `assignWorker` hint.
  - `ResourceBar` — pulses the hinted resource slot's icon+label **alpha** (color stays owned by flash/starvation logic).
