...
- **`TileMap.js`**: 20×20 grid (current test size — real maps may be much larger). Tile records: `{ col, row, type, buildingId, isField, ownedBy, height, isRamp, isRoad, resources }`. `height` is 0–3; `isRamp` is true for GRASS tiles at a height transition; `isRoad` is true when a road tile has been placed (tile type stays GRASS so villagers remain walkable). `generate()` runs: weighted random fill → smoothing pass → heightmap (dome hills with slope-limiting) → ramp placement → iron deposit. `_placeIronDeposit(topCol, topRow)` stamps a 2×2 IRON block at uniform height (levelled to the minimum height of the 4 tiles), then scatters ~3 individual IRON tiles in adjacent cells. Called in `generate()` at `col+row ≈ 28–34` (deep map, requires exploration). Exports `MAP_SIZE` and `MAX_TILE_HEIGHT`. Exposes `getTile`, `getNeighbors` (4-dir), `getNeighbors8` (8-dir), `isInBounds`.
...
- **`walkable.js`**: `isWalkable(tile)` — GRASS tile with no building. Road tiles keep `type === 'GRASS'` and no `buildingId`, so they are walkable. `heightMoveCost(fromTile, toTile)` — returns `FLAT_MOVE_COST` (1) for same height, `RAMP_MOVE_COST` (2) for a one-level transition via a ramp, `Infinity` for cliffs or height diff > 1. `randomWalkableTile(tileMap, exclude?)` — random walkable tile from the full grid. `randomWalkableTileNear(tileMap, centerCol, centerRow, radius)` — random walkable tile within Manhattan radius; used by warriors for bounded wander.
