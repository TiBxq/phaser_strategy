...
- **`WarriorEntity.js`**: Single warrior sprite. Mirrors `VillagerEntity` but uses `'sprite-warrior'` texture (red-armored 10×14 sprite). Stores `_homeCol/_homeRow` (Barracks anchor); picks wander targets via `randomWalkableTileNear` with `WANDER_RADIUS=4`. `MOVE_DURATION=600`, idle pause `800–2000 ms`. **March methods**: `marchTo(col, row, onArrived)` cancels the active wander timer, kills tweens, runs A* to target, and walks the path — firing `onArrived` when the first warrior arrives (or immediately if unreachable). `marchHome()` calls `marchTo(_homeCol, _homeRow, …)` then resumes normal wandering.
- **`WarriorRenderer.js`**: Maintains a `Map<buildingUid, WarriorEntity[]>` pool. Listens to `WARRIORS_CHANGED { buildingUid, building }` → `_syncPool()` adds/removes entities to match `building.assignedVillagers`. Spawn location: `randomWalkableTileNear` with radius 3, fallback to `randomWalkableTile`. Listens to `BUILDING_REMOVED` → destroys all entities for that uid. **`marchAllTo(targetCol, targetRow, onFirstArrived)`** marches every warrior; a `fired` flag ensures `onFirstArrived` fires only once (on first arrival). **`marchAllHome()`** calls `marchHome()` on every entity.
...
- **Attack mechanic**: clicking a `banditCampTile` (or the camp building sprite) shows `TileInfoPanel` with an **Attack Camp** button. Requires ≥ 5 total warriors across all Barracks. On click, `UI.js` calls `warriorRenderer.marchAllTo(campCol, campRow, cb)` — warriors march via A* (ignoring `banditClaimed`); the first to arrive triggers `BanditCampSystem.clear()`, destroys camp sprites via `BanditRenderer.clearCamp()`, emits `BANDIT_CAMP_CLEARED { clearedTiles }`, then all warriors march home.
...
- **Events**: `BANDIT_CAMP_ATTACK_REQUEST` (emitted by TileInfoPanel on button click), `BANDIT_CAMP_CLEARED { clearedTiles }` (emitted by UI.js after clear; MapRenderer subscribes to remove tints).
