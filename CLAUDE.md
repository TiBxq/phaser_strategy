...
- **Attack mechanic**: clicking a `banditCampTile` (or the camp building sprite) shows `TileInfoPanel` with an **Attack Camp** button. Requires ≥ 5 total warriors across all Barracks. On click, `UI.js` sets up a `FOG_UPDATED` listener and calls `warriorRenderer.marchAllTo(campCol, campRow, doClear, onAllFailed)`. Warriors use `isWalkableForMarch` + `marchMoveCost` (high-cost building passthrough) and reveal fog radius 1 around each step. **Camp clears when any `banditCampTile` enters `FOG_VISIBLE`** (primary trigger — the camp is destroyed the moment warriors see it). Physical arrival at the camp tile is kept as a fallback `doClear` in case the reveal fires late. If every warrior fails to path (terrain fully blocked), `onAllFailed` fires a "path blocked" notification instead of clearing the camp.
...
- **Events**: `BANDIT_CAMP_ATTACK_REQUEST` (emitted by TileInfoPanel on button click), `BANDIT_CAMP_CLEARED { clearedTiles }` (emitted by UI.js after clear; MapRenderer subscribes to remove tints), `BANDIT_THREAT_STATE_CHANGED { state, stealAmount }` (emitted by BanditThreatSystem on state transitions and steal-tier changes), `BANDIT_PILLAGE_TARGET { buildingUid: string|null }` (emitted when a new pillage target is selected or cleared).
